...
const { calculateNightDifferential } = require("../helpers/calculateNightDifferential");
...
exports.punchOut = async function (req, res) {
    try {
        ...
        const lateMinutes =
            actualIn > shiftStart
                ? Math.floor(
                    (actualIn.getTime() - shiftStart.getTime()) /
                    (1000 * 60)
                )
                : 0;
        ...
        const undertimeMinutes =
            actualOut < shiftEnd
                ? Math.floor(
                    (shiftEnd.getTime() - actualOut.getTime()) /
                    (1000 * 60)
                )
                : 0;

        const { totalWorkedHours } =
            computeHours(timeIn, timeOut);

        const regularHours =
            Math.min(totalWorkedHours, shiftHours);

        const overtimeHours =
            totalWorkedHours > shiftHours
                ? Number(
                    (totalWorkedHours - shiftHours).toFixed(2)
                )
                : 0;
...
exports.thisWeek = async function (req, res) {
    try {
        ...
        const weeklySummary = snap.docs.reduce((acc, doc) => {
            const d = doc.data();
